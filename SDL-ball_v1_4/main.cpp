...
int test = 0;
void writeSettings();
void writeSettings_test();
bool initScreen();
void initNewGame();
void initNewGame_test();
int q = 0;
void pauseGame();
void resumeGame();
...
struct texProp {
  ...
  string fileName; //Quite the ugly.. This will be set by readTexProps();
};
...
class textureClass {
  ...
  public:
    ...
    int compare_test(textureClass text)
    {

   if (age!=text.age) {
      return -1;
   }
   else if (dir != text.dir) {
      return -1;
   }
   else if (lastframe != text.lastframe) {
      return -1;
   }
   else return 0;
   }
...
void play_test() // checks if the paddle moves right
{
    int col_test=0,row_test=0;
    bool firstFrame_test = 1;
    uint lastframe_test = 1000;
    uint frame_test = 1;
    texProp prop_test;
    prop_test = prop;
    texPos pos_test;
int err = 0;
    for (int t = 0; t < 8; t++) //copy the data tot test variable
    {
           pos_test[t] = pos[t];
    }
    uint f_test = 1;
      ...
      {
        for(col_test=0; col_test < prop_test.cols; col_test ++) // calculate the different margins and copy to test variables
        {
            ...
            if(prop_test.padding)   // padding correction
            {
              pos_test[0] += 1.0 / prop_test.pxw;
              pos_test[1] += 1.0 / prop_test.pxh;

              pos_test[2] -= 1.0 / prop_test.pxw;
              pos_test[3] += 1.0 / prop_test.pxh;

              pos_test[4] -= 1.0 / prop_test.pxw;
              pos_test[5] -= 1.0 / prop_test.pxh;

              pos_test[6] += 1.0 / prop_test.pxw;
              pos_test[7] -= 1.0 / prop_test.pxh;
            }
            if (pos_test[0] != pos[0])  // check if there are errors between the test code and the original code
            {err++;}
            if (pos_test[1] != pos[1])
            {err++;}
            if (pos_test[2] != pos[2])
...
cout << "Start test : Paddle texture" << endl; // print errors
            cout << "if paddletexture of picture is changing => picture changed err:" << err << endl;// 0 errors => y height stays the same.
    ...
    void draw()
    {
      ...
      if(!dead)
      {
        ...
        glBegin( GL_QUADS );
            ...
            glTexCoord2f(tex.pos[2], tex.pos[3]); glVertex3f( width,height, 0.0f ); // draws the position taken from the tex file
    ...
    void draw()
    {
      ...
      for(i=0; i < len; i++)
      {
        ...
        if(active[i])
        {
          ...
          glBegin( GL_QUADS );
            glTexCoord2f(tex->pos[0],tex->pos[1]);glVertex3f( -width*s[i], height*s[i], 0.00 );
            glTexCoord2f(tex->pos[2],tex->pos[3]);glVertex3f(  width*s[i], height*s[i], 0.00 );
            glTexCoord2f(tex->pos[4],tex->pos[5]);glVertex3f(  width*s[i],-height*s[i], 0.00 );
            glTexCoord2f(tex->pos[6],tex->pos[7]);glVertex3f( -width*s[i],-height*s[i], 0.00 );
          glEnd( );
        }
      }
    }
...
class ball : public moving_object {
  ...
  void hit(GLfloat c[])
  {
    ...
    void move_test()  // test to check if the ball change direction(change sign) when hit against the wall
    {   cout << "Start test Move()" << endl;
    if(posx+width > 1.6)
    {
       if(xvel == -1)
       {
                   cout << "Error: right border hit, sign didn't change: " <<xvel << endl; // if the ball hits the wall and the sign isn't changed give an error
       }
    }
    if(posx+width < 1.6)
    {
       if(xvel == 1)
       {
                   cout << "Error: left border hit, sign didn't change: " <<xvel << endl;
       }
    }
    cout << "Stop test Move()" << endl;
    }
  void move()
  {


    if(posx+width > 1.6 && xvel > 0.0) // right border
    {
      soundMan.add(SND_BALL_HIT_BORDER, posx);
      xvel *= -1;
    } else if(posx-width < -1.6 && xvel < 0.0) // left border
    {
      soundMan.add(SND_BALL_HIT_BORDER, posx);
      xvel *= -1;
    } else if(posy+width > 1.25 && yvel > 0.0) // upper border
    {
      soundMan.add(SND_BALL_HIT_BORDER, posx);
      yvel *= -1;
    } else if(posy-width < -1.24) // under paddle
    {
      active=0;
...
class ballManager {
  ...
  public:
    ...
    void getSpeed_test()    // The value of the ballspeed should never be less than 0.
    {

      if(var.averageBallSpeed < 0) // if the value of ballspeed is smaller than zero indicate an error.
      {
                      cout << "Start test getSpeed" << endl;

              cout << "error in var.averageBallspeed - value less than 0" << endl;
                        cout << "End test getSpeed" << endl;
        ...
        if(b[i].active)
        {
          switch(powerup)
          {
            ...
            case PO_NORMALBALL: // normal balls
              ...
              b[i].setspeed(difficulty.ballspeed[player.difficulty]);
              break;
            case PO_EXPLOSIVE: //exploding bricks
              b[i].explosive=1;
...
class powerupManager {
  ...
  public:
    ...
    void spawn_test( pos velocity,int type) // boundary test
    {



      for(i=0; i < MAXPOWERUPS; i++) // if the powerups spawn on locations bigger than 2 they appear out of the screen. Powerups spawn locations should stay smaller than 2 and bigger than -2
      {
        if(!p[i].active)
        {
          p[i].gravity = 0.7;
...
void initNewGame_test() // check if the player difficulty is set right.
{
    cout<< "Start test initNewgame"<<endl;
     switch(player.difficulty)
  {
  case EASY:
      if (player.coins == 600 && player.lives == 5) //regarding different variables the player difficulty should be set accordingly
        cout << "no error in player difficulty: EASY" << endl;
      break;
    case NORMAL:
      if (player.coins == 0 && player.lives == 3)
        cout << "no error in player difficulty: NORMAL" << endl;
      break;
      break;
    case HARD:
      if (player.coins == 0 && player.lives == 3)
                cout << "no error in player difficulty: HARD" << endl;
      break;
      break;
  }
      cout<< "Stop test initNewgame"<<endl;
...
void writeSettings_test()  // check if the settings are read from the text file
{
  ifstream conf;
    int noerr = 0;
  string line, set, val;
  conf.open(privFile.settingsFile.data()); //homeDirFiles.settingsFile.data()

  if(conf.is_open())
  {
        cout << "Start test writesettings"<< endl;

     while(!conf.eof())  //read file until there are no more lines
    {
      getline(conf, line);
      if(line.find('=') != string::npos)
      {
        set=line.substr(0,line.find('='));
        val=line.substr(line.find('=')+1);
        if(set=="eyecandy")   // check each file entry for errors
        {
            if (setting.eyeCandy != atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="resx")
        {
            if (setting.resx!=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="resy")
        {
            if (setting.resy!=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="showbg")
        {
          if (setting.showBg !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="fullscreen")
        {
          if (setting.fullscreen !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="particlecollide")
        {
          if (setting.particleCollide !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="sound")
        {
          if (setting.sound !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="stereo")
        {if (setting.stereo !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="controlmaxspeed")
        {
         if (setting.controlMaxSpeed!=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="controlaccel")
        {
          if (setting.controlAccel !=atof(val.data()))
            {
                   noerr++;
            }
        } else if(set=="controlstartspeed")
        {
          if (setting.controlStartSpeed!=atof(val.data()))
            {
                   noerr++;
            }
        } else if(set=="rightkey")
        {
          if (setting.keyRight !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="leftkey")
        ...
        {
          if (setting.keyBuyPo !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="prevkey")
        {
         if (setting.keyPrevPo !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="joyenabled")
        {
          if (setting.joyEnabled !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="joyisdigital")
        {
          if (setting.joyIsDigital !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="joycalhighjitter")
        {
          if (setting.JoyCalHighJitter !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="joycallowjitter")
        {
          if (setting.JoyCalLowJitter !=atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="joycalmax")
        {
          if (setting.JoyCalMax ==atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="joycalmin")
        {
          if (setting.JoyCalMin ==atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="lvltheme")
        {
         if (setting.lvlTheme == val)
            {
                   noerr++;
            }
        } else if(set=="sndtheme")
        {
          if (setting.sndTheme == val)
            {
                   noerr++;
            }
        } else if(set=="gfxtheme")
        {
         if (setting.gfxTheme == val)
            {
                   noerr++;
            }
        } else if(set=="startingdifficulty")
        {
          if (player.difficulty == atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="showclock")
        {
          if (setting.showClock == atoi(val.data()))
            {
                   noerr++;
            }
        } else if(set=="fps")
        {
          if (setting.fps !=atoi(val.data()))
            {
                   noerr++;
            }
            }
        ...
        cout << "number of errors:"<<noerr<<"' in settings.cfg" << endl; // print number of errors: the difference between the variables in the game and the values read from the file
      }
    }
            cout << "Stop test writesettings"<< endl;
  ...
  while(!var.quit)
  {
    ...
    if(!var.titleScreenShow)
    {
        ...
        if(var.effectnum == -1)
        {
          ...
          if(var.transition_half_done)
          {
            ...
            if(!var.idiotlock)
            {
              ...
              sprintf(txt, "Level %i",player.level+1);
      ...
      if(gVar.newLevel)
      {
        ...
        if(test == 1)
        {
            loadlevel1_test(bricks);
        }
        initlevels(bricks,texLvl);
        if(test == 1)
        {
            initlevel1_test(bricks);
        }
        gVar.gameOver=0;
        gVar.newLife=1;
        pMan.clear();
