...
class ball : public moving_object {
  ...
  void hit(GLfloat c[])
  {
    ...
    if(posx+width > 1.6)
    {
       if(xvel == -1)
       {
                   cout << "Error: right border hit, sign didn't change: " <<xvel << endl;
       }
    }
    if(posx+width < 1.6)
    {
...
class powerupManager {
  ...
  public:
    ...
    void spawn_test( pos velocity,int type)
    {
...
void initNewGame_test()
{
  ...
  {
  case EASY:
      if (player.coins == 600 && player.lives == 5)
  ...
  if(conf.is_open())
  {
     ...
     while(!conf.eof())
    {
      ...
      if(line.find('=') != string::npos)
      {
        ...
        cout << "number of errors:"<<noerr<<"' in settings.cfg" << endl;
      }
    }
