...
float numberof_test = 1000;
    char val_test =0;
        for(int n = 0; n < numberof_test; n++)
        {
            val_test = selectRandomPowerup();
            switch ( val_test )
            {

            case '2' : Shrink_test++; break;
    ...
    cout << "Most:" << endl;
    cout << "Shrink: " << Shrink_test/numberof_test << endl;
...
void loadlevel1_test(brick bricks[])
{
...
{
                if(bricks[(i)/2].powerup != testvar[i])
                {
...
void initlevel1_test(brick bricks[])
{   int errinit = 0;
    bool inter_test= 0;
